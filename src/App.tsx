...
function App() {
  ...
  // LIVE DEBUG CONTROLS
  const [debugX, setDebugX] = useState(-0.045)
  const [debugY, setDebugY] = useState(0.675)
  ...
  return (
    <>
      <div id="overlay-ui">
        ...
        {/* Live Debug Panel (Moved to root below) */}
          ...
          <div style={{display: 'flex', alignItems: 'center', gap: '5px'}}>
            ...
            <input type="range" min="-1.50" max="1.00" step="0.001" value={debugX} onChange={e => setDebugX(parseFloat(e.target.value))} style={{width: '90px'}} />
            <button onClick={() => setDebugX(-0.045)} style={{padding: '0 5px'}}>↺</button>
          </div>
          <div style={{display: 'flex', alignItems: 'center', gap: '5px'}}>
            Y offset: 
            <input type="number" step="0.001" value={debugY} onChange={e => setDebugY(parseFloat(e.target.value))} style={{width: '60px'}} />
            <input type="range" min="-1.0" max="2.0" step="0.001" value={debugY} onChange={e => setDebugY(parseFloat(e.target.value))} style={{width: '90px'}} />
            <button onClick={() => setDebugY(0.675)} style={{padding: '0 5px'}}>↺</button>
          </div>
          <div style={{display: 'flex', alignItems: 'center', gap: '5px'}}>
            Z offset: 
            <input type="number" step="0.001" value={debugZ} onChange={e => setDebugZ(parseFloat(e.target.value))} style={{width: '60px'}} />
            <input type="range" min="-2.0" max="1.0" step="0.001" value={debugZ} onChange={e => setDebugZ(parseFloat(e.target.value))} style={{width: '90px'}} />
      ...
      {/* Live Debug Panel (Root Execution for Z-Index Exemption) */}
      <div style={{ position: 'fixed', top: 20, left: 20, background: 'rgba(0,0,0,0.8)', padding: '15px', color: 'white', borderRadius: '8px', zIndex: 2147483647, pointerEvents: 'auto', display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '11px', fontFamily: 'monospace', maxHeight: '95vh', overflowY: 'auto' }}>
        ...
        <strong style={{color: '#fa0'}}>Geometric Slicer</strong>
        <div style={{display: 'flex', alignItems: 'center', gap: '5px'}}>
          Min X: 
          <input type="number" step="0.01" value={sliceMinX} onChange={e => setSliceMinX(parseFloat(e.target.value))} style={{width: '60px'}} />
          <input type="range" min="-1.0" max="1.0" step="0.01" value={sliceMinX} onChange={e => setSliceMinX(parseFloat(e.target.value))} style={{width: '90px'}} />
